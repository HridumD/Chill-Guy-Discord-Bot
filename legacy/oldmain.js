...
// Secrets
const { TOKEN, CLIENT_ID, GUILD_ID, CLAIM_CHANNEL_ID } = require('../dupeconfig.json');

// Firebase
admin.initializeApp({
  credential: admin.credential.cert(require('../chillguydupefirebasekey.json'))
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    // Update database after selecting classes
...
client.on('guildMemberAdd', async member => {
  ...
  if (roles.length) {
    ...
    await db.collection('users').doc(member.id)
      .collection('guilds').doc(GUILD_ID)
      .set({ classes: saved, updatedAt: new Date() }, { merge: true });
  }
});

/* ---------------- RESTORE ROLES ON STARTUP ---------------- */
client.once('ready', async () => {
  ...
  try {
    ...
    for (const member of members.values()) {
      const userSnap = await db.collection('users').doc(member.id)
        .collection('guilds').doc(GUILD_ID).get();

      const savedClasses = userSnap.exists ? userSnap.data().classes || [] : [];

      if (!savedClasses.length) continue;

      const classSnap = await db.collection(GUILD_ID).get();
      const rolesToAdd = classSnap.docs
        .filter(doc => savedClasses.includes(doc.id))
      ...
      if (rolesToAdd.length) {
        ...
        // Ensure database matches roles
        const actualClasses = classSnap.docs
          .filter(doc => rolesToAdd.includes(doc.data().roleId))
          .map(doc => doc.id);

        await db.collection('users').doc(member.id)
          .collection('guilds').doc(GUILD_ID)
          .set({ classes: actualClasses, updatedAt: new Date() }, { merge: true });
      }
    }

    console.log('Restored roles from database for all members.');
  } catch (err) {
    console.error('Error restoring roles on startup:', err);
  }
});
